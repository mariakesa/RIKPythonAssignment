...
def genereeri_osanikud(n_osanikud_uniq=200, max_n_osanikud=5):
    ...
    osanikud=np.random.choice(np.arange(1,n_osanikud_uniq+1), osanike_n_per_osauhing, replace=False)
    return osanikud
...
def total_capital_calculation(osauhingud_ilma_kapitalita, osauhing_assoc_1, osauhing_assoc_2):
    ...
    osauhingud_ilma_kapitalita.index = np.arange(1, len(osauhingud_ilma_kapitalita) + 1)
